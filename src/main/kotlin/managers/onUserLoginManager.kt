...
class onUserLoginManager(private val project: Project)
{
...
//    }
    ...
    fun checkinsertion()
    {
        ...
        for (i in documentText.indices)
        {
            ...
            if(line.contains("OnUserLoginProperties.put(\"USer Property_name \", \"value\");"))
            {
                context_exsit=true

            }
            if(line.contains("clevertapDefaultInstance.onUserLogin(OnUserLoginProperties);"))
            {
                onuserlogin_method_exist=true

            }
            if(line.contains("HashMap<String, Object> OnUserLoginProperties = new HashMap<String, Object>();"))
            {
                hashmap_obj=true
